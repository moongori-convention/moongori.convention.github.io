...
$(function () {
	...
	$(".btn-reset").click(function () {
		...
		[].forEach.call(jsonData, function (element) {
			...
			if (element.display == "Y") {
				...
				$cate_list2.find("input").prop("checked", true);
			} else {
				$cate_list2.addClass("unenabled");
				$cate_list2.find("input").prop("checked", false);
			}
		})
	});
	// 카테고리 설정 > 취소 버튼
	...
	// 카테고리 설정 > 체크박스
	$(".category-list.type2 .category-item label").on('click', function(){
		var $child_input = $(this).find("input[type='checkbox']");
		var $child_name = $(this).find(".category-name");
		if ($child_input.prop('checked')) {
			category_cnt++;
			$(this).parent().removeClass("unenabled");
			toast("<strong>" + $child_name.text() + "</strong> 카테고리가 설정되었습니다.");
		} else {
			category_cnt--;
			$(this).parent().addClass("unenabled");
			toast("<strong>" + $child_name.text() + "</strong> 카테고리가 해제되었습니다.");
		}

		$(".category-description .description2 span").text(category_cnt);
	});
	$(".category-list.type2 .category-move").on({
		touchstart : function(){
			$(this).closest(".category-item").addClass("ing");
		},
		touchend : function() {
			$(this).closest(".category-item").removeClass("ing");
		}
	});
	...
	$(".category-list.type2").sortable({ // drag (category-list)
		...
		//helper : "clone",
		stop:function(e,ui){
			$(ui.item).removeClass("ing");
			toast("<strong>" + $(ui.item).text() + "</strong> 카테고리 순서가 변경되었습니다.");
		}
	}).disableSelection();
...
function activeCategory() { // scroll, click : add active class
	$("[id^=cate-]").not(".hide").each(function (index) {
		...
		if (($(this).offset().top - headerHeight - 20 - $(window).scrollTop()) < 0) {
			...
			$(".category-list-innerwrap").scrollLeft(this_pos);
		}
	});
}
...
function settingCategory() { // 'btn-confirm' click : create json
	...
	for (let i = 0; i < 19; i++) {
		...
		data.display = $category_this.find("input[type='checkbox']").prop("checked") ? 'Y' : 'N';
...
function categoryTitleDetach() {
	$(".category-list.type1 .category-name").each(function(){
		var text = $(this).text();
