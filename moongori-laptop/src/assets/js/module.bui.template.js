...
// 탭 기본
var tabs = new buiTab('[data-bui-tab="normal"]');
...
Detectizr.browser.name === 'ie' ? formCheckBox('.form.radio, .form.checkbox') : null;

// form textfield util setup
function formFieldset(selector) {
	var forms = document.querySelectorAll(selector);
	Array.prototype.forEach.call(forms, function(form) {
		var formElem = form.querySelector('.form-elem:not([readonly])'); if (!formElem) return;
		var formUtil = form.querySelector('.form-util'); if (!formUtil) return;
		var formClear = form.querySelector('.form-clear'); if (!formClear) return;
...
formFieldset('.form.textfield');

// form textarea util setup
function formTextarea(selector) {
	var forms = document.querySelectorAll(selector);
	Array.prototype.forEach.call(forms, function(form) {
		var formElem = form.querySelector('.form-elem'); if (!formElem) return;
		var formUtil = form.querySelector('.form-util'); if (!formUtil) return;
...
var fromResizeTag = function(elem) {
...
// formAddFile
var formAddFile = function(inputEl) {
...
// 파일 업로더
var fileUploader = function(selector) {
...
localNavigations('.local-navi .navi-item');
...
// 쿠폰 다운로드
var donloadCoupon = function(elem) {
...
// 가격 설정
var buiFormRange = function(selector) {
	var settings = {
...
function scrollNavigations(navigations, margin) {
	...
	Array.prototype.forEach.call(items, function(item, index) {
		...
		item.addEventListener('click', function(e) {
			e.preventDefault();
			scrollEvent = false;
			gsap.to(window, {duration: 0.4, scrollTo: {y: target, offsetY: tabs.clientHeight + targetGap}});
	...
	document.addEventListener('wheel',  function() {scrollEvent = true;});
	document.addEventListener('keydown',  function() {scrollEvent = true;});
}


// 상품상세 > 리뷰 목록 > 좋아요
var itemLikeit = function(elem) {
...
if (document.querySelector('#totalPayment')) {
	ScrollTrigger.create({
		...
		start: 'top 192',
		end: () => `bottom top+=${document.querySelector('#totalPayment').offsetHeight + 192}`,
		toggleClass: 'active',
		onLeave: function(self) {document.querySelector('.content-body').classList.add('on-leave');},
		onEnterBack: function(self) {document.querySelector('.content-body').classList.remove('on-leave');},
		// markers: true
	});
}
